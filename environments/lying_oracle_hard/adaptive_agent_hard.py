...
class AdaptiveAgentHard:
    ...
    def __init__(
        self,
        detection_window: int = 20,  # SIMPLIFIED: Smaller window for faster detection
        trust_to_distrust_threshold: float = 0.18,  # SIMPLIFIED: Adjusted to max observed rate (~20%)
        distrust_to_trust_threshold: float = 0.10,  # Hysteresis: much lower to stay in DISTRUST
        min_observations: int = 15,  # SIMPLIFIED: Fewer observations needed
    ):
        """
    ...
    def _update_lying_confidence(self, turn: int):
        """
        SIMPLIFIED VERSION: Use only contradiction rate as detection statistic.
        
        Following the challenge suggestion: "Track a simple statistic 
        (moving average of reward, contradiction rate, etc.)"
        
        Using ONLY contradiction rate - the simplest and most reliable signal.
        """
        if len(self.history) < self.min_observations:
            return
        
        # SIMPLIFIED: Only contradiction rate
        ...
        contradiction_rate = contradiction_count / len(recent_history)
        
        # DEBUG: Print if we have any contradictions
        if turn % 50 == 0 and turn > self.min_observations:
            print(f"  [DEBUG turn {turn}] Contradictions in last {len(recent_history)}: "
                  f"{contradiction_count}, rate={contradiction_rate:.3f}, "
                  f"total_contradictions={self.contradictions}")
        
        # Direct mapping: contradiction rate IS the lying confidence
        # No complex weighting, no smoothing, no multi-signal fusion
        # Just: "What % of recent queries caused contradictions?"
        self.lying_confidence = contradiction_rate
        
        # Optional: Small smoothing to avoid jitter (but much less aggressive)
        # alpha = 0.7  # High alpha = trust new data more
        # self.lying_confidence = alpha * contradiction_rate + (1 - alpha) * self.lying_confidence
    ...
    def update(self, k: int, oracle_response: bool, reward: float, mode_switched: bool):
        ...
        # Check if a contradiction occurred in THIS turn
        # by comparing contradiction count with previous turn
        contradiction = False
        if len(self.history) > 0:
            prev_contradictions = self.history[-1]["contradictions"]
            if self.contradictions > prev_contradictions:
                contradiction = True
                # DEBUG: Print when contradiction detected
                print(f"  ✓ [CONTRADICTION at turn {len(self.history)}] prev={prev_contradictions}, now={self.contradictions}")
