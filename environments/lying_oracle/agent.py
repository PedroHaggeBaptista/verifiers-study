...
This agent:
1. Starts with binary search (assuming truthful oracle)
2. Tracks contradictions and reward statistics
3. Detects when oracle starts lying
4. Switches to inverted binary search
"""
...
class AdaptiveAgent:
    """
    An agent that adapts when it detects the oracle is lying.
    
    Strategy:
    - Use binary search initially (truthful oracle assumption)
    - Track moving average of rewards
    - Track contradictions (impossible oracle responses)
    - When anomalies detected, switch to "lying mode" (invert oracle answers)
    """
    ...
    def __init__(
        ...
        Args:
            detection_window: Window size for moving average
            contradiction_threshold: Number of contradictions to trigger mode switch
            reward_drop_threshold: Reward drop to trigger investigation
        """
    ...
    def select_action(self, turn: int, last_response: bool = None) -> int:
        ...
        Returns:
            k value to query
        """
        # First turn: start in the middle
        if turn == 0:
        ...
        if self.low <= self.high:
            ...
            k = (self.low + 100) // 2  # Recovery strategy
    ...
    def _should_switch_mode(self, turn: int) -> bool:
        ...
        1. Multiple contradictions detected
        2. Significant reward drop
        3. Moving average indicates problem
        """
        # Strategy 1: Too many contradictions
...
def run_episode(
    ...
    """
    Run a complete episode with the adaptive agent using the environment.
    
    Args:
        hidden_number: The number to find
        max_turns: Maximum number of turns
        lying_threshold: Turn when oracle starts lying
        verbose: Print debug information
        env: LyingOracleEnv instance (if None, creates one)
